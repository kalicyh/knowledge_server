...
export default {
  ...
  methods: {
    ...
    async fetchMySQLData() {
      try {
        const result = await fetchMySQLData('/numbers/data');
        this.mysqlData = result.numbers;
      } catch (error) {
    ...
    async fetchInfo() {
      try {
        const result = await fetchInfo('/numbers/info');
    ...
    async uploadData() {
      try {
        await uploadData(this.file, '/numbers/upload', '/numbers/progress', (progress) => {
    ...
    async uploadnewData() {
      try {
        await uploadData(this.file, '/numbers/overwrite_upload', '/numbers/progress', (progress) => {
          this.progress = progress; // 更新进度状态
        }, async () => {
          await this.fetchInfo(); // 上传完成后获取更新的信息
        });
